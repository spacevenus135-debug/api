...
def detect_gate(shop_url: str, client: "TLSClient") -> str:
    ...
    Priority: /payments/config JSON → homepage → checkout JS assets
    ...
    detected = "shopify_payments"  # safe default

    try:
        # ── Check 1: /payments/config — most authoritative ─────────
        try:
            r = client.get(
            ...
            if r.status_code == 200 and body:
                # Check for authorize.net specifically first
                if any(p.lower() in body.lower() for p in GATE_SIGNATURES["authorize.net"]):
                    detected = "authorize.net"
                elif any(p.lower() in body.lower() for p in GATE_SIGNATURES["stripe"]):
                    detected = "stripe"
                elif any(p.lower() in body.lower() for p in GATE_SIGNATURES["braintree"]):
                    detected = "braintree"
                elif any(p.lower() in body.lower() for p in GATE_SIGNATURES["square"]):
                    detected = "square"
                elif "shopify_payments" in body.lower() or "shopify" in body.lower():
                    detected = "shopify_payments"
                logger.info(f"Gate via /payments/config: {detected}")
        except Exception as e:
            logger.warning(f"Gate /payments/config failed: {e}")

        # ── Check 2: checkout page — only if payments/config unclear ─
        if detected == "shopify_payments":
            try:
                ...
                # useAuthorizeUrl in JS filename = definitive authorize.net
                js_files = re.findall(
                    r'["\']([^"\']*useAuthorize[^"\']*\.js[^"\']*)["\']',
                    checkout_html
                )
                if js_files:
                    detected = "authorize.net"
                    logger.info("Gate via checkout JS filename: authorize.net (useAuthorizeUrl)")
                else:
                    # Check other gates — skip paypal
                    for gate in ["stripe", "braintree", "square", "adyen", "cybersource"]:
                        if any(p.lower() in checkout_html.lower()
                               for p in GATE_SIGNATURES[gate]):
                            detected = gate
                            logger.info(f"Gate via checkout HTML: {gate}")
                            break
            except Exception as e:
                logger.warning(f"Gate checkout scan failed: {e}")

    except Exception as e:
        logger.warning(f"Gate detection error for {domain}: {e}")

    with _gate_cache_lock:
        _gate_cache[domain] = detected

    logger.info(f"Final gate [{domain}]: {detected}")
    return detected
