...
GATE_SIGNATURES: Dict[str, List[str]] = {
    "authorize.net": [
        ...
        "authorizenet", "Accept.js", "anet", "authorize_net",
        "apiLoginID", "clientKey", "AuthorizeNet",
    ],
    "stripe": [
        "js.stripe.com", "stripe.com/v3", "Stripe(", "stripe_publishable_key",
        "pk_live_", "pk_test_", "stripe-js", "stripeElements",
    ],
    "braintree": [
        "js.braintreegateway.com", "braintree.client", "braintreegateway",
        "client_token", "braintree-web",
    ],
    "paypal": [
        "paypal.com/sdk", "paypalobjects.com", "paypal-button",
        "PAYPAL", "paypalConfig", "ShopifyPaypalV4",
    ],
    "shopify_payments": [
        "shopify_payments", "shopifycs.com", "shopifyinc.com",
        "vault.shopify.com", "pay.shopify.com",
    ],
    "square": [
        "squareup.com", "square.com/payments", "SqPaymentForm",
        "Square.js",
    ],
    "adyen": [
        "adyen.com", "checkoutshopper", "adyen-checkout",
        "AdyenCheckout",
    ],
    "cybersource": [
...
def detect_gate(shop_url: str, client: "TLSClient") -> str:
    ...
    try:
        ...
        # ── Check 3: scan JS asset names from checkout ─────────────
        if detected == "shopify_payments":
            try:
                r = client.get(
                ...
                # look for useAuthorizeUrl specifically (most reliable signal)
                if "useAuthorizeUrl" in checkout_html or "authorize" in checkout_html.lower():
                    detected = "authorize.net"
                    logger.info("Gate detected via checkout HTML: authorize.net")
                else:
                    for gate, patterns in GATE_SIGNATURES.items():
