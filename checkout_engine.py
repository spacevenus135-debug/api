...
GATE_CACHE_TTL = 3600  # 1 hour TTL
_gate_cache_time: Dict[str, float] = {}

def detect_gate(shop_url: str, client: "TLSClient") -> str:
    """
    Detect the payment gateway used by a Shopify store.
    Default: shopify_payments — only override if strong signal found.
    Cache TTL: 1 hour.
    """
    ...
    with _gate_cache_lock:
        if domain in _gate_cache:
            age = time.time() - _gate_cache_time.get(domain, 0)
            if age < GATE_CACHE_TTL:
                logger.info(f"Gate cache hit: {domain} → {_gate_cache[domain]}")
                return _gate_cache[domain]
            else:
                # Expired — remove
                del _gate_cache[domain]
                _gate_cache_time.pop(domain, None)

    # Default is shopify_payments — only change if STRONG signal
    detected = "shopify_payments"

    try:
        # ── /payments/config — authoritative source ────────────────
        try:
            r = client.get(
            ...
            if r.status_code == 200 and body:
                ...
                # Strong signals only — must be explicit
                if "authorize.net" in body_lower or "authorizenet" in body_lower:
                    detected = "authorize.net"
                elif '"gateway":"stripe"' in body_lower or '"gateway": "stripe"' in body_lower:
                    detected = "stripe"
                elif "braintreegateway" in body_lower:
                    detected = "braintree"
                # shopify / paypal / generic = stay as shopify_payments
                logger.info(f"Gate via /payments/config [{domain}]: {detected}")
        except Exception as e:
            logger.warning(f"Gate /payments/config error: {e}")

        # ── checkout JS assets — only if payments/config unclear ───
        if detected == "shopify_payments":
            try:
                ...
                # useAuthorizeUrl in JS *filename* = definitive signal
                js_files = re.findall(
                    r'["\']([^"\']*useAuthorize[^"\']*\.js[^"\']*)["\']',
                    checkout_html,
                )
                if js_files:
                    detected = "authorize.net"
                    logger.info("Gate via JS filename: authorize.net")
            except Exception:
                pass

    except Exception as e:
        logger.warning(f"Gate detection error [{domain}]: {e}")

    with _gate_cache_lock:
        _gate_cache[domain] = detected
        _gate_cache_time[domain] = time.time()
