...
async def check_get(
    cc:    str = Query(..., description="Card string: number|mm|yyyy|cvv"),
    ...
    card_val, err = _validate_card(cc)
    if err:
        err.CC = cc
        return err

    url_val, err = _validate_url(site)
    if err:
        err.CC = cc
        err.Site = site
        return err
...
@app.post("/shopify", response_model=CheckResponse, tags=["check"])
